...
int main() {
  ...
  (void)chain.add({.stage = "features", .id = "rage", .modifier = [](int dmg) { return dmg + 2; }});
  (void)chain.add(
      {.stage = "equipment", .id = "magic_sword", .modifier = [](int dmg) { return dmg + 1; }});
  (void)chain.add(
      {.stage = "final", .id = "resistance", .modifier = [](int dmg) { return dmg / 2; }});
  ...
  (void)fight2.add(
      {.stage = "features", .id = "rage", .modifier = [](int dmg) { return dmg + 2; }});
  (void)fight2.add(
      {.stage = "final", .id = "resistance", .modifier = [](int dmg) { return dmg / 2; }});
