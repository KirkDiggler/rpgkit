// Tutorial 06 — the Bus: the game loop announces, subscribers respond.
//
// This is tutorial 05's game, grown: a Bus connects everything. The game loop
// publishes turn events instead of managing everything inline. Subscribers
// respond — block resets, combat log entries — without the loop knowing what
// they are.
//
...
//
// What's new since 05:
//   1. A Bus — one object, passed everywhere, connecting publishers and
//      subscribers that never meet.
//   2. turn.ended — a notification topic. The game loop publishes it;
//      subscribers respond (block reset, combat log).
//   3. Block reset is now a subscriber, not inline code in the loop.
//   4. Combat log — another subscriber, pure flavor, zero mechanics.
// What did NOT change: the player's cards, the goblin AI, the damage chains
// (still Chain<int>), the turn structure. No Effects, no Bleed, no Rend.
//
// Next: tutorial 07 migrates damage from Chain<int> to Chain<DamageAttempt>
// and introduces chained topics — subscribers that contribute modifiers to
// a resolution instead of just observing.
...
// ---- Topics — static facts about this game ----

// A notification topic: the turn loop broadcasts, effects and subscribers
// respond. No return value, no transformation — just "this happened."
...
// ---- Player phase (unchanged from tutorial 05) ----
...
// ---- Goblin phase (unchanged from tutorial 05, Bus passed through) ----
...
int main() {
  ...
  while (true) {
    ...
    // The game loop's interaction with the Bus: publish the turn-ended event.
    ...
    // Check death from the goblin phase (subscriber may have modified HP).
    if (hero.hp <= 0) {
      std::cout << "\nYou crumple. The goblin shuffles your deck into its loot bag.\n";
      return 0;
    }
